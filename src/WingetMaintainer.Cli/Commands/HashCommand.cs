...
internal static class HashCommand
{
    ...
    {
        Option<DirectoryInfo> pathOption = new("--path", "Path to the manifest folder.") { IsRequired = true };
        pathOption.AddAlias("-p");

        Command command = new("hash", "Compute the SHA-256 manifest hash and installer hashes for a folder.")
        {
            pathOption,
        };
    ...
    private static void Run(DirectoryInfo path)
    {
        ...
        Console.WriteLine(JsonSerializer.Serialize(
            new { result.ManifestHash, result.InstallerHashes }, JsonOptions));
    }
}
