...
internal static class SubmitCommand
{
    ...
    {
        Option<DirectoryInfo> pathOption = new("--path", "Manifest folder to submit.") { IsRequired = true };
        ...
        Option<SubmissionTool> toolOption = new("--tool", () => SubmissionTool.Komac, "Submission tool.");
        Option<string?> tokenOption = new("--token", "GitHub token (falls back to GITHUB_TOKEN or WINGET_PAT).");
        Option<string?> resolvesOption = new("--resolves", "Issue number this PR resolves (komac).");
        ...
        {
            pathOption, idOption, versionOption, toolOption, tokenOption, resolvesOption, prTitleOption,
        };
        ...
        command.SetHandler(async (context) =>
        {
            ...
            ProcessResult result = await service.SubmitAsync(options, context.GetCancellationToken());
