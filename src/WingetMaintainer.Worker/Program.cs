...
WorkerOptions options =
    builder.Configuration.GetSection(WorkerOptions.SectionName).Get<WorkerOptions>() ?? new WorkerOptions();
...
app.MapGet("/api/jobs/next", async (string host, IValidationQueue queue, CancellationToken cancellationToken) =>
{
    QueuedJob? job = await queue.DequeueNextAsync(host, cancellationToken);
    return job is null ? Results.NoContent() : Results.Ok(job);
});

app.MapPost("/api/jobs/{id:int}/result",
...
app.MapGet("/api/runs", async (WingetMaintainerDbContext dbContext, CancellationToken cancellationToken) =>
    Results.Ok(await dbContext.PackageRuns
        .OrderByDescending(run => run.Id)
        .Take(50)
        .ToListAsync(cancellationToken)));

app.MapGet("/api/state/{id}", async (string id, IPackageStateStore store, CancellationToken cancellationToken) =>
{
