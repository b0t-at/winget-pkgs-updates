...
public sealed class ValidationQueueTests : IDisposable
{
    ...
    public async Task DequeueNextAsync_ReturnsOldestPendingAndMarksInProgress()
    {
        ...
        int firstId = await queue.EnqueueAsync(runId, "Contoso.App", @"C:\m\1", CancellationToken.None);
        await queue.EnqueueAsync(runId, "Contoso.App", @"C:\m\2", CancellationToken.None);

        QueuedJob? job = await new ValidationQueue(database.CreateContext())
            .DequeueNextAsync("runner-1", CancellationToken.None);
        ...
        using WingetMaintainerDbContext context = database.CreateContext();
        ValidationJob persisted = context.ValidationJobs.Single(candidate => candidate.Id == firstId);
    ...
    {
        int runId = await SeedRunAsync();
        int jobId = await new ValidationQueue(database.CreateContext())
            .EnqueueAsync(runId, "Contoso.App", @"C:\m\1", CancellationToken.None);
        await new ValidationQueue(database.CreateContext()).DequeueNextAsync("runner-1", CancellationToken.None);

        await new ValidationQueue(database.CreateContext()).CompleteAsync(
            new JobResult { JobId = jobId, Status = ValidationJobStatuses.Passed, Host = "runner-1", ExitCode = 0, LogRef = "log://x" },
    ...
    {
        Func<Task> act = () => new ValidationQueue(database.CreateContext()).CompleteAsync(
            new JobResult { JobId = 999, Status = ValidationJobStatuses.Failed }, CancellationToken.None);
